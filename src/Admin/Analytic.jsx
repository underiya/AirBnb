        ...
        // CARD
          <div>
            <div className='rounded-lg w-[100%] h-[50%] overflow-hidden relative ' >
              <img className='h-[100%] w-[100%] ' src={el.images[currentIndex]} alt="" />
              <div className='text-[14px] font-[400] rounded-full bg-gray-100 py-[2px] px-[15px] absolute top-[10px] left-[10px] ' >Guest Favourite</div>
              <div className='absolute top-[10px] right-[10px] text-[24px] font-[700] ' >{<CiHeart  />}</div>

              <span onClick={prevImage} className=' cursor-pointer absolute top-1/2 transform -translate-y-1/2 left-[10px] text-[12px] rounded-full bg-gray-100 flex items-center justify-start py-[10px] px-[10px] text-black font-[500] ' >{<FaAngleLeft />}</span>
              <span onClick={nextImage} className=' cursor-pointer absolute top-1/2 transform -translate-y-1/2  right-[10px] text-[12px] rounded-full bg-gray-100 flex items-center justify-start py-[10px] px-[10px] text-black font-[500] ' >{<FaAngleRight />}</span>
              ...
              <div className=' absolute h-[70px] w-[70px] bottom-[10px] left-[10px] bg-gray-200 rounded-r-lg shadow-lg flex items-center justify-center ' >
                <div className='rounded-full overflow-hidden h-[40px] w-[40px] ' ><img className='w-[100%] h-[100%]   ' src="https://img.freepik.com/premium-vector/avatar-icon0002_750950-43.jpg" alt="" /> </div>
              </div>

              <div className='bottom-[20px] w-[50%] mx-auto left-1/2 transform transalet-x-1/2 absolute flex gap-x-[5px] ' >
                {
                  el.images.map((el,i)=>(
                    <div className={`h-[5px] w-[5px] rounded-full ${currentIndex===i?'bg-white':' bg-gray-300 bg-opacity-50 '}   `} ></div>
                  ))
                }
              </div>
            ...
            <div className='font-[400] text-gray-600 text-[16px] ' >{el.description}</div>
            <div className='text-[16px] font-[400] ' ><span className='font-[600] ' >{el.price} /</span>night</div>
          </div>
        ))
      }
    </div>
